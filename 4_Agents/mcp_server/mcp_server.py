...
if MCP_SDK_AVAILABLE:
    ...
    async def handle_calculate(arguments: dict[str, Any]) -> Sequence[TextContent]:
        ...
        if operation == "add":
            ...
            if b == 0:
                return [TextContent(type="text", text=f"Error: Division by zero")]
            result = a / b
        else:
    ...
    async def handle_get_missing_students(arguments: dict[str, Any]) -> Sequence[TextContent]:
        """This request can be copied from a web browser in developper mode"""
        import requests
        CLASS_ID = ""
        EDUSIGN_COOKIE = ""
        headers = {
            ...
            'Authorization': f'Bearer {EDUSIGN_COOKIE}',
            'Content-Length': '653',
            'Referer': 'https://edusign.app/',
        }

        data = '{"studentIds":["student_id_1", "student_id_2"]}' # 

        response = requests.post(f'https://api.edusign.fr/professor/students/getManyAttendanceList/{CLASS_ID}', headers=headers, data=data)
    ...
    TOOL_HANDLERS: dict[str, Callable[[dict[str, Any]], Awaitable[Sequence[TextContent]]]] = {
        ...
        "get_absent": handle_get_missing_students,
    }
    ...
    async def list_tools() -> list[Tool]:
        ...
        return [
            ...
            Tool(
                name="handle_get_missing_students",
                description="get list of missing students during last session",
                inputSchema={
                    "type": "object",
                    "properties": {},
                    "required": []
